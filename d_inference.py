...
if __name__== '__main__':
    ...
    base_dir = '/disk/sites_dataset/' #'/gpfs/projects/nct01/nct01127/dataset/'
    ...
    inference_sampler = torch.utils.data.distributed.DistributedSampler(inference_dataset,num_replicas=int(os.environ['SLURM_NTASKS']),rank=rank, shuffle=False)
    ...
    ddp_model.load_state_dict(
        torch.jit.load(CHECKPOINT_PATH, map_location=None))
    ...
    with torch.no_grad():
        for image in inference_data_loader:
            ...
            if (itr % sampling_rate == 0):
                filename = str(rank) + '_' + str(itr) + '_' + 'sites_results.pickle'
